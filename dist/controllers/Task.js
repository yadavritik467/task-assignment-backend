...
export const createTask = catchAsync(async (req, res, next) => {
    const { title, description, assignedTo, dueDate, priority, status, assignedName, } = req.body;
    if (!title)
...
export const updateTask = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const { title, description, assignedTo, dueDate, status, priority, assignedName, } = req.body;
...
export const deleteMyTask = catchAsync(async (req, res, next) => {
    ...
    if (!myTask)
        return next(new AppError("Task already Deleted", 404));
...
export const getAllTasks = catchAsync(async (req, res, next) => {
    ...
    const allTasks = await Task.find({ ...searchFilter })
        ...
        .skip(skip)
        .limit(limit);
    const assignedFilter = req?.user.role === ROLES.USER ? { assignedTo: req?.user?._id } : {};
